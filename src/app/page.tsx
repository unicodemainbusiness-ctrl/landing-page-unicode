...
const services = [
    { title: 'Desain UI/UX', icon: Palette, desc: 'Merancang antarmuka yang intuitif dan menarik untuk pengalaman pengguna terbaik.' },
    { title: 'Pengembangan Aplikasi', icon: AppWindow, desc: 'Membangun aplikasi web dan mobile yang fungsional dan skalabel.' },
    { title: 'Website & E-Commerce', icon: Globe, desc: 'Membuat website profil perusahaan dan toko online yang profesional.' },
    { title: 'Desain Sistem', icon: Component, desc: 'Menciptakan komponen desain yang konsisten untuk skalabilitas produk.' },
    { title: 'Wireframing & Prototyping', icon: PenTool, desc: 'Memvisualisasikan alur pengguna dan struktur aplikasi sebelum pengembangan.' },
    { title: 'Identitas Merek Digital', icon: SwatchBook, desc: 'Membangun identitas visual yang kuat untuk merek Anda di dunia digital.' },
    { title: 'Konsultasi Teknologi', icon: Bot, desc: 'Memberikan wawasan dan strategi teknologi untuk mendukung tujuan bisnis Anda.' },
    { title: 'Manajemen Versi (Git)', icon: GitBranch, desc: 'Mengelola kode secara efisien menggunakan Git untuk kolaborasi tim yang lancar.' }
];

const stacks_logo = [
  { name: 'Tailwind', logo: <SiTailwindcss color='#06B6D4' className='text-5xl' /> },
  { name: 'Next.js', logo: <SiNextdotjs color='#FFFFFF' className='text-5xl' /> },
  { name: 'React', logo: <SiReact color='#61DAFB' className='text-5xl' /> },
  { name: 'NestJS', logo: <SiNestjs color='#E0234E' className='text-5xl' /> },
  { name: 'Node.js', logo: <SiNodedotjs color='#339933' className='text-5xl' /> },
  { name: 'PostgreSQL', logo: <SiPostgresql color='#336791' className='text-5xl' /> },
  { name: 'TypeScript', logo: <SiTypescript color='#3178C6' className='text-5xl' /> },
  ...
  { name: 'JavaScript', logo: <SiJavascript color='#F7DF1E' className='text-5xl' /> },
  { name: 'Supabase', logo: <SiSupabase color='#3ECF8E' className='text-5xl' /> },
  ...
  { name: 'Prettier', logo: <SiPrettier color='#F7B93E' className='text-5xl' /> },
  { name: 'ShadCN UI', logo: <SiShadcnui color='#FFFFFF' className='text-5xl' /> },
  { name: 'DaisyUI', logo: <SiDaisyui color='#A855F7' className='text-5xl' /> },
  { name: 'Auth0', logo: <SiAuth0 color='#EB5424' className='text-5xl' /> }
]

const stacks = [
  'Next.js', 'NestJS', 'React', 'TailwindCSS', 'Figma', 'TypeScript',
  'PostgreSQL', 'MongoDB', 'Node.js', 'Prisma', 'Git', 'Vercel',
...
const Header = ({ refs }: any) => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-gray-900/80 backdrop-blur-sm border-b border-gray-800' : 'bg-transparent'}`}>
      <div className='container mx-auto px-4 sm:px-6 lg:px-8 py-4'>
        <div className='flex items-center justify-between'>
          ...
          <nav className='hidden lg:flex items-center space-x-8'>
            {navLinks.map(link => (
              <button key={link.name} onClick={() => scrollToSection(link.ref)} className='text-sm font-medium text-gray-300 transition-colors hover:text-white'>
                {link.name}
              </button>
            ))}
          </nav>
          ...
          <div className='lg:hidden'>
            <button title='button' onClick={() => setIsMenuOpen(!isMenuOpen)} className='p-2 text-gray-300 rounded-md hover:bg-gray-800'>
        ...
        {isMenuOpen && (
          <div className='mt-4 lg:hidden bg-gray-800/90 rounded-lg p-4'>
            <nav className='flex flex-col space-y-4'>
              {navLinks.map(link => (
                <button key={link.name} onClick={() => scrollToSection(link.ref)} className='text-sm text-left font-medium text-gray-300 transition-colors hover:text-white'>
                  {link.name}
                </button>
              ))}
              <button className='btn btn-primary rounded-md'>
...
export default function UnicodeLandingPage() {
  ...
  const sectionRefs = { layananRef, spesialisasiRef, teknologiRef, prosesRef, kontakRef };
  ...
  return (
    <>
      ...
      <div className='min-h-screen font-sans text-white bg-gray-900'>
        <div className='pointer-events-none fixed inset-0 z-50 transition duration-300 hidden lg:block' style={{ background: `radial-gradient(600px at ${mousePosition.x}px ${mousePosition.y}px, rgba(79, 70, 229, 0.15), transparent 80%)` }}></div>
        <div className='relative overflow-hidden'>
          ...
          <main className='relative z-10'>
            <section className='h-screen relative pt-32 pb-20 text-center sm:pt-40 lg:pt-48 overflow-hidden flex flex-col justify-center'>
              ...
              <div className='container relative z-10 px-4 mx-auto sm:px-6 lg:px-8'>
                ...
                <div className='flex flex-col items-center justify-center max-w-xs mx-auto mt-8 sm:flex-row sm:max-w-none'>
                  <button onClick={() => scrollToSection(layananRef)} className='btn btn-primary btn-lg rounded-lg'>
                    Jelajahi Layanan
            ...
            <section className='bg-indigo-400 py-4 overflow-hidden'>
              <Marquee gradient={false} speed={40} pauseOnHover={true} className='gap-8'>
                {stacks.map((item, index) => (
                  <div key={index} className='flex items-center gap-2 text-black font-semibold text-lg mx-6'>
                    <span>{item}</span>
            ...
            <section ref={spesialisasiRef} className='py-20 sm:py-24 bg-transparent'>
              <div className='container px-4 mx-auto sm:px-6 lg:px-8'>
                ...
                <motion.div className='grid gap-8 sm:grid-cols-2 lg:grid-cols-4' variants={containerVariants} initial='hidden' whileInView='visible' viewport={{ once: true, amount: 0.3 }}>
                  {services.map((service, index) => (
                    <motion.div key={index} variants={cardVariants as any} className='relative p-6 bg-gray-800/50 border border-gray-700 rounded-xl transition-all hover:border-indigo-500/60 hover:bg-gray-800'>
                      ...
                      <a href='#' className='text-sm text-indigo-400 hover:text-indigo-300 inline-flex items-center'>
            ...
            <section ref={teknologiRef} className='relative py-28 overflow-hidden text-white flex flex-col items-center justify-center'>
              <style jsx>{`
                @keyframes rotate-circle {
                  0% { transform: rotate(0deg); }
                  100% { transform: rotate(360deg); }
                }
                .spin-circle { animation: rotate-circle 40s linear infinite; }
              `}</style>
              <div className='relative w-[400px] h-[400px] mb-20'>
                <div className='absolute inset-0 spin-circle origin-center top-56'>
                  {stacks_logo.map((stack, index) => {
                    ...
                    return (
                      <div key={index} className='absolute w-16 h-16 p-2 rounded-xl bg-white/10 backdrop-blur-md shadow-md transition-transform duration-300 hover:scale-110 flex items-center justify-center' style={{ transform: `rotate(${angle}deg) translate(${radius}px) rotate(-${angle}deg)`, transformOrigin: 'center center', top: '50%', left: '50%', marginTop: '-2rem', marginLeft: '-2rem' }}>
                        {stack.logo}
                      </div>
                    )
                  })}
              ...
              <div className='flex flex-col gap-4 relative bottom-56 items-center'>
                <motion.h2 ref={ref} initial={{ clipPath: 'inset(0 50% 0 50%)', opacity: 0 }} animate={ isInView ? { clipPath: 'inset(0 0% 0 0%)', opacity: 1, transition: { duration: 2, ease: 'linear', clipPath: { delay: 0.2, duration: 2, ease: 'easeInOut' } } } : {} } className='text-3xl md:text-4xl font-bold text-center whitespace-nowrap overflow-hidden'>
                  Membangun dengan Tumpukan <br /> Teknologi Terbaik
                </motion.h2>
                <p className='text-gray-400 mt-3 mb-6 text-center max-w-lg'>
                  Jelajahi teknologi web modern yang kami gunakan untuk membangun pengalaman digital yang kuat, skalabel, dan menakjubkan.
                </p>
                <button onClick={() => scrollToSection(kontakRef)} className='w-1/2 btn btn-primary rounded-md'>
                  Hubungi Kami
                </button>
              </div>
            </section>

            <section ref={prosesRef} className='relative py-24 bg-gradient-to-br overflow-hidden'>
              <div className='relative z-10 container mx-auto px-4 text-center'>
                ...
                <div className='mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3'>
                  {[
                    { number: '1', step: 'Langkah 01', title: 'Perencanaan & Konsultasi', desc: 'Konsultasikan kebutuhan Anda dan dapatkan rekomendasi strategis dari tim ahli kami.' },
                    { number: '2', step: 'Langkah 02', title: 'Desain & Pengembangan', desc: 'Kami mulai mengerjakan desain UI/UX dan fungsionalitas yang dibutuhkan secara iteratif.' },
                    { number: '3', step: 'Langkah 03', title: 'Peluncuran & Dukungan', desc: 'Website selesai, kami bantu proses deploy ke domain Anda dan memberikan dukungan teknis.' }
                  ].map((item, index) => (
                    <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.2 }} viewport={{ once: true }} key={index} className='relative group py-8 bg-white/5 border border-white/10 rounded-xl p-6 overflow-hidden backdrop-blur-md transition duration-300 hover:shadow-lg'>
                      ...
                      <div className='relative z-10'>
                        <p className='text-sm text-indigo-400 font-medium'>{item.step}</p>
            ...
            <section ref={kontakRef} className='py-24 bg-transparent'>
              <div className='container px-4 mx-auto sm:px-6 lg:px-8 text-center'>
                <h2 className='text-3xl font-bold text-white sm:text-4xl'>
                  Mari Ciptakan Proyek <span className='text-indigo-400'>Luar Biasa</span> Bersama!
                </h2>
                <p className='mt-4 text-lg text-gray-400 max-w-2xl mx-auto'>
                  Siap untuk membawa ide Anda ke level berikutnya? Hubungi kami untuk memulai kolaborasi dan wujudkan proyek digital impian Anda.
                </p>
                <motion.div className='flex flex-wrap justify-center gap-3 mt-12' initial='hidden' whileInView='visible' viewport={{ once: true, amount: 0.3 }} transition={{ staggerChildren: 0.08 }}>
                  {keywords.map((word, idx) => (
                    <motion.div key={idx} variants={{ hidden: { opacity: 0, y: -60, rotate: -5 }, visible: { opacity: 1, y: 0, rotate: 0, transition: { type: 'spring', damping: 10, stiffness: 100 } } }} className='px-4 py-2 text-sm font-medium text-white bg-gray-800 border border-gray-700 rounded-full hover:bg-indigo-500 hover:border-indigo-400 hover:text-white transition-colors'>
                      {word}
                    </motion.div>
                  ))}
                </motion.div>
                <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2, duration: 0.4 }} viewport={{ once: true }} className='mt-12'>
